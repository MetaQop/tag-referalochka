...
async def join_age_yes(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(
        ...
        "<i>Напиши ответ текстом (например: от друга, реклама, поиск Telegram...)</i>"
    )
    await c.answer()
...
async def join_source_answer(m: Message, state: FSMContext):
    await state.update_data(source=m.text or "—")
    await state.set_state(JoinRequest.q3_agree)
    await m.answer(
        "━━━━━━━━━━━━━━\n"
        "❓ <b>Вопрос 3 из 3</b>\n\n"
        "Прочитай правила канала:\n\n"
        "📌 <b>Правила:</b>\n"
        "• Контент только для совершеннолетних (18+)\n"
        "• Запрещён спам и реклама\n"
        "• Уважай других участников\n"
        "• Запрещена публикация личных данных без согласия\n"
        "• Администрация вправе исключить за нарушения без предупреждения\n\n"
