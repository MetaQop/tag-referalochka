...
async def get_inactive_users_to_remind_stage(inactive_days: int, stage: int):
    """Пользователи, которым нужно отправить напоминание stage-й стадии.
    
    Условия:
    - есть invite_link (зарегистрированы)
    - не выполнили задание (completed = 0)
    - прошло >= inactive_days с момента last_active
    - ещё не получили напоминание этой стадии (reminder_stage < stage)
    """
...
async def mark_reminder_stage(user_id, stage: int):
    """Обновляем стадию напоминания."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET reminder_stage = ? WHERE user_id = ? AND reminder_stage < ?",
            (stage, user_id, stage)
        )
        await db.commit()
...
async def get_inactive_users_to_remind(inactive_days=3):
    return await get_inactive_users_to_remind_stage(inactive_days, 1)

async def mark_reminder_sent(user_id):
    await mark_reminder_stage(user_id, 1)
